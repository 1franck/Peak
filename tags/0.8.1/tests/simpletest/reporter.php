...
class HtmlReporter extends SimpleReporter {
     ...
     */
    function _getCss() {
        return "body { width:2000px; font:12px Courier New, Monaco, monospace; }".
                "p { margin:10px 15px; }".
    ...
    function paintFooter($test_name) {
        ...
        if(($this->getFailCount() >= 1) && ($this->getPassCount() >= 1)) {
            $fail_percent = round(($this->getFailCount() / $this->getPassCount()) * 100);
            $pass_percent = 100 - $fail_percent; 
        }
        else { $pass_percent = 100; $fail_percent = 0; }
        ...
        print "<strong>" . $this->getPassCount() . "</strong> passes($pass_percent%), ";
        print "<strong>" . $this->getFailCount() . "</strong> fails($fail_percent%) and ";
