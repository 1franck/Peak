 ...
 * @see Peak_Pattern
 */
require_once 'Peak/Pattern.php';

/**
 * @category   Peak
 * @package    Peak_Pattern
 * @subpackage UnitTests
 * @version    $Id$
 */
class Peak_PatternTest extends PHPUnit_Framework_TestCase
{
	//test for method text()
    function testText()
    {
        $this->assertTrue((bool)Peak_Pattern::text('ssg4gredfg',false),'not detected as text without space');
        $this->assertTrue((bool)Peak_Pattern::text('ssg4gr edfg',true),'not detected as text with space');
        $this->assertTrue((bool)Peak_Pattern::text('ssg4gr edfg gf dfg',true),'not detected as text with space');
        
        $this->assertFalse((bool)Peak_Pattern::text('ssg4gr @ edfg',true),'not detected as text with space');
        $this->assertFalse((bool)Peak_Pattern::text('@¤46546'),'not detected as text without space');
        $this->assertFalse((bool)Peak_Pattern::text('ssdg redfg'),'not detected as text without space');      
    }
    ...
    function testEmail()
    {
        ...
        $emails = array('abcdef123@hotmail.com' => true,
                        ...
                        //'abcdef123@hotmail.1' => false, //fail
                        'abcdef123@hotmail.' => false,
                        //'test@hotmail' => false, //fail
                        );
        
        foreach($emails as $email => $expected) {
            if($expected) $this->assertTrue((bool)Peak_Pattern::email($email), $email.' detected as invalid email');
            else $this->assertFalse((bool)Peak_Pattern::email($email), $email.' detected as valid email');
        }
    ...
    function testIp()
    {
        ...
        foreach($ips as $ip => $expected) {
            if($expected) $this->assertTrue((bool)Peak_Pattern::ip($ip), $ip.' detected as invalid ip address');
            else $this->assertFalse((bool)Peak_Pattern::ip($ip), $ip.' detected as valid ip address');
        }
    }
    ...
    function testUrl()
    {
        ...
        foreach($urlToTest as $url => $expected) {
            if($expected) $this->assertTrue((bool)Peak_Pattern::url($url),'url '.$url.' no detected properly');
            else $this->assertFalse((bool)Peak_Pattern::url($url), 'url '.$url.' detected as url');
        }
    }
